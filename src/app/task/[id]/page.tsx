...
export default function TaskPage() {
  ...
  const pollTask = useCallback(async () => {
    try {
      // First, trigger RunningHub status check via POST
      ...
      // If still running, also fetch full task data via GET
      if (pollData.status === "running" || pollData.status === "queued") {
        const getRes = await fetch(`/api/task/${taskId}`);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // If still running, immediately start polling RunningHub
  ...
  const statusConfig: Record<string, { label: string; color: string; icon: string }> = {
    queued: { label: "In Queue", color: "text-yellow-400", icon: "⏳" },
    uploading: { label: "Uploading", color: "text-blue-400", icon: "📤" },
    running: { label: "Generating", color: "text-purple-400", icon: "⚡" },
    success: { label: "Complete", color: "text-green-400", icon: "✅" },
          ...
          <>
            ...
            {(task.status === "queued" || task.status === "uploading" || task.status === "running") && (
              <div className="bg-white/5 border border-white/10 rounded-2xl p-8 mb-8 text-center">
                <div className="animate-pulse">
                  ...
                  <h2 className="text-xl font-semibold mb-2">AI is working its magic...</h2>
                  <p className="text-gray-400 mb-4">This usually takes 5-15 minutes</p>
                  ...
                  <p className="text-gray-600 text-xs mt-4">Auto-refreshing every 10 seconds...</p>
                </div>
              </div>
            )}
                ...
                <div className="flex flex-wrap items-center justify-center gap-4 mt-6">
                  ...
                  <button
                    onClick={() => {
                      if (navigator.share) {
                        navigator.share({
                          title: "Check out this AI-animated video!",
                          text: "Made with Animaker.AI — turn any photo into a dancing video!",
            ...
            {task.status === "failed" && (
              <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-6 mb-8">
                ...
                <p className="text-gray-400 text-sm">{task.errorMessage || "An unexpected error occurred. Your credit has been refunded."}</p>
