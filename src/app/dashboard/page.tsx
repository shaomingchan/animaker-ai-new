...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch("/api/user/me")
      ...
      .then((data) => {
        ...
        else if (data?.error) router.replace("/login");
      })
      .catch(() => {
        router.replace("/login");
      })
  ...
  const statusBadge: Record<string, { label: string; cls: string }> = {
    ...
    uploading: { label: t("dashboard.status.uploading"), cls: "bg-blue-500/10 text-blue-400" },
    running: { label: t("dashboard.status.running"), cls: "bg-purple-500/10 text-purple-400" },
    success: { label: t("dashboard.status.success"), cls: "bg-green-500/10 text-green-400" },
    failed: { label: t("dashboard.status.failed"), cls: "bg-red-500/10 text-red-400" },
  };
          ...
          <>
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-12">
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                <p className="text-gray-500 text-sm mb-1">{t("dashboard.creditsRemaining")}</p>
              ...
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                <p className="text-gray-500 text-sm mb-1">{t("dashboard.currentPlan")}</p>
                <p className="text-3xl font-bold capitalize">{user?.plan ?? "free"}</p>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                <p className="text-gray-500 text-sm mb-1">{t("dashboard.totalVideos")}</p>
            ...
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold">{t("dashboard.yourVideos")}</h2>
            ...
            {!user?.tasks?.length ? (
              <div className="text-center py-16 bg-white/5 border border-white/10 rounded-2xl">
                <p className="text-gray-500 mb-4">{t("dashboard.noVideos")}</p>
