    ...
    <main className="min-h-screen bg-black text-white">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-2xl mx-auto">
            {[
              ...
              <div key={plan.name} className={`relative p-8 rounded-2xl border ${plan.popular ? 'border-purple-500 bg-purple-500/10' : 'border-white/10 bg-white/5'}`}>
                ...
                <Link href={`/api/checkout?product=${plan.productId}`} className={`block text-center py-3 rounded-full font-medium transition ${plan.popular ? 'bg-purple-500 text-white hover:bg-purple-600' : 'bg-purple-400/80 text-white hover:bg-purple-400'}`}>
                  Get Credits
                </Link>
              </div>
            ))}
      ...
      <section className="py-20 px-6 border-t border-white/5">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-3xl font-bold text-center mb-16">{faqData.title}</h2>
